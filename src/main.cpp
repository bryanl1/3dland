/**
 * Adapted from the OpenGL tutorial at https://learnopengl.com/.
 * Adapted from work partially by Neal Terrell, 2024.
...
Mesh constructMesh(const std::vector<Vertex3D>& vertices, const std::vector<uint32_t>& faces) {
	...
	// Second vertex attribute u and v. //(3 * sizeof(float))
...
void fromAssimpMesh(const aiMesh* mesh, std::vector<Vertex3D>& vertices,
	...
	for (size_t i = 0; i < mesh->mNumVertices; i++) {
		...
		//auto& meshVertex = mesh->mVertices[i];//auto& texCoord = mesh->mTextureCoords[0][i];
		//vertices.push_back({texCoord.x, texCoord.y});
		
		vertices.push_back(Vertex3D(mesh->mVertices[i].x, mesh->mVertices[i].y, mesh->mVertices[i].z, mesh->mTextureCoords[0][i].x, mesh->mTextureCoords[0][i].y));
...
Mesh triangle() {
	...
	StbImage grass;
	grass.loadFromFile("models/grass.jpg");
	m.texture = generateTexture(grass);
	return m;
}
...
Mesh bunny() {
	...
	StbImage texture;
	texture.loadFromFile("models/bunny_textured.jpg");//bunny_textured
	//texture.loadFromFile("models/grass.jpg");
...
Mesh test() {
	// Load the bunny, but vertically flip its UV coordinates, because the creator uses 
	// (0, 0) as the lower LEFT corner of texture space.
	Mesh obj = assimpLoad("models/backpack.obj", true);
	StbImage texture;
	texture.loadFromFile("models/diffuse.jpg");//bunny_textured
	//texture.loadFromFile("models/grass.jpg");
	obj.texture = generateTexture(texture);
	return obj;
}
